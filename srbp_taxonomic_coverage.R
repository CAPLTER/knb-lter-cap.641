...
# get SRBP taxa from srbp_birds object
srbp_taxa <- srbp_birds %>% 
  select(common_name) %>% 
...
# resolve a tsn
all_taxa <- identify_resolvable_taxa(srbp_taxa)
...
srbpTaxa <- set_taxonomicCoverage(resolved_taxa, expand = T, db = 'itis')

# set taxonomicCoverage element
coverage@taxonomicCoverage <- c(srbpTaxa)
